...
from miniso_studio.application.reporting_qwen_strict import (
    render_full_report_qwen_strict,
    render_opening_report_qwen_strict,
    to_view_qwen_strict,
)
...
app = FastAPI(
    title="Trend2SKU Qwen 实时动态爆款产品决策工作台",
...
def live_index() -> FileResponse:
    frontend = Path(settings().project_root) / "frontend" / "index-qwen-strict.html"
...
def live_health() -> dict:
    ...
    return {
        ...
        "model": model,
        "decision_mode": "qwen_strict_dynamic_with_deterministic_guardrails",
    }
...
def live_result(
    ...
    return to_view_qwen_strict(artifacts)
...
def live_report(
    ...
    try:
        markdown = (
            render_opening_report_qwen_strict(artifacts)
            if kind == "opening"
            else render_full_report_qwen_strict(artifacts)
        )
