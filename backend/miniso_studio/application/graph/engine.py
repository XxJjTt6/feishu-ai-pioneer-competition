...
class StateGraph:
    ...
    def run(
        ...
        resume: bool = False,
        checkpoint_snapshot: Optional[Tuple[PipelineState, str]] = None,
    ) -> PipelineState:
        if resume and checkpointer is not None:
            loaded = checkpoint_snapshot or checkpointer.load(thread_id)
            if loaded is None:
        ...
        while current != END:
            ...
            with span_ctx:
                try:
                    ...
                    raise
                except Exception as exc:  # noqa: BLE001 - 可降级的节点错误即信息
